...
class BaliDiagramActivity : DiagramActivity() {
    ...
    override fun onCustomItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.action_bali_7days -> return updateViewPager(0)
            R.id.action_bali_wind -> return updateViewPager(1)
            R.id.action_bali_humidity -> return updateViewPager(2)
            R.id.action_30_days -> return updateViewPager(3)
            R.id.action_bali_paderborn -> return updateViewPager(4)
            R.id.action_last_year -> return updateViewPager(5)
        }
        return false
    }
}
